...
![auto-zoom](https://img.shields.io/badge/auto--zoom-v1.0.0-%23C50008?logo=npm)
...
| `reference` | `string | HTMLElement` | 自动缩放的参照元素. 默认为：`false`, 将会观察窗口尺寸. |
| `designSize`  | `number[]` | 预期的设计稿尺寸。默认为：`[1920, 1080]`。|
| `transform` | `object | Function` | CSS transform 的配置，支持 `origin`、`translate` 选项。默认为：`{ origin: "0 0", translate: "translate(0,0)" }`。|
...
type AutoZoomOptions {
  target:string | HTMLElement; // 要自动缩放的目标元素。
  reference:string | HTMLElement | false; // 自动缩放的参照元素。
  designSize: [number, number]; // 目标的期望设计稿尺寸。
  transform: { // CSS transform-origin 和 transform 的 translate 选项。
    origin: string,
    translate: string | (zoom:number, [referenceWidth, referenceHeight], [designWidth, designHeight]) => string; // 支持使用自定义函数控制缩放
  };
